...
def build_tts_adapter(
    *,
    ...
    """Upstream-compatible entry point (Settings-based config, ElevenLabs support)."""
    from opentalking.core.config import get_settings

    settings = settings or get_settings()
    provider = getattr(settings, "normalized_tts_provider", _provider())
    ...
    if provider in _QWEN_RT or provider in _COSY_WS or provider in _MINIMAX_SSE or provider in _SAMBERT:
        return create_tts_adapter(sample_rate=sample_rate, chunk_ms=chunk_ms)
