...
def reset_avatar_speech_state(avatar_state: Any) -> None:
    ...
    if "gesture_pending_semantic_state" in extra:
        extra["gesture_pending_semantic_state"] = None
    if "gesture_active_text" in extra:
        extra["gesture_active_text"] = ""
    if "gesture_used_semantic_states" in extra:
        extra["gesture_used_semantic_states"] = set()
    if "gesture_text_cursor" in extra:
        extra["gesture_text_cursor"] = 0
    if "gesture_pending_text_cursor" in extra:
        extra["gesture_pending_text_cursor"] = 0
    if "gesture_target_frame_indices" in extra and "gesture_current_state" in extra and "gesture_config" in extra:
        config = extra.get("gesture_config")
        current_state = str(extra.get("gesture_current_state"))
        if hasattr(config, "frame_indices") and current_state in config.frame_indices:
            extra["gesture_target_frame_indices"] = [int(config.frame_indices[current_state])]
            extra["gesture_target_state_labels"] = [current_state]
        else:
            extra["gesture_target_frame_indices"] = []
            extra["gesture_target_state_labels"] = []
    if "gesture_transition_mask" in extra:
        extra["gesture_transition_mask"] = None
