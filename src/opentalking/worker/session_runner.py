...
from opentalking.tts.coqui import CoquiXTTSAdapter
...
class SessionRunner:
    ...
    async def _prewarm_tts(self) -> None:
        ...
        try:
            ...
            if isinstance(tts, CoquiXTTSAdapter):
                prewarm_started_at = _time.perf_counter()
                await tts.warmup(text=self._tts_prewarm_text)
                log.info(
                    "xtts prewarm done in %.0fms for session %s",
                    (_time.perf_counter() - prewarm_started_at) * 1000,
                    self.session_id,
                )
        except Exception:  # noqa: BLE001
            log.warning("xtts prewarm failed for session %s", self.session_id, exc_info=True)
        ...
        async with self._speak_lock:
            ...
            try:
                ...
                if self.avatar_state is not None:
                    ...
                    if isinstance(getattr(self.avatar_state, "extra", None), dict):
                        self.avatar_state.extra["gesture_active_text"] = speech_text
