...
class FlashTalkRunner:
    ...
    def __init__(
        ...
        *,
        ...
        self._tts_reference_audio = tts_reference_audio.strip()
    ...
    def _tts_runtime_voice(self) -> str:
        if self._tts_provider == "xtts":
            return self._tts_reference_audio or self._tts_voice
    ...
    def _build_tts_adapter(self, *, sample_rate: int) -> Any:
        ...
        if self._tts_provider == "xtts":
            reference_audio = Path(
                self._tts_reference_audio or settings.tts_clone_reference_audio
            ).expanduser().resolve()
    ...
    async def _prewarm_tts(self, *, sample_rate: int) -> None:
        try:
            ...
            if isinstance(tts, CoquiXTTSAdapter):
                ...
                await tts.warmup("你好")
                log.info(
                    "FlashTalk XTTS prewarm done in %.0fms for session %s (ref=%s)",
                    (_t.perf_counter() - started_at) * 1000.0,
                    self.session_id,
                    self._tts_runtime_voice(),
                )
        except Exception:
            log.warning(
                "FlashTalk TTS prewarm failed for session %s (provider=%s)",
                self.session_id,
                self._tts_provider,
                exc_info=True,
            )
    ...
    async def prepare(self) -> None:
        ...
        if self._tts_provider == "xtts":
            await self._prewarm_tts(sample_rate=16000)
