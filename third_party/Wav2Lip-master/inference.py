...
def get_mouth_blend_mask(height, width):
...
def blend_mouth_only(pred, original):
	mask = get_mouth_blend_mask(pred.shape[0], pred.shape[1])
...
def main():
	...
	for i, (img_batch, mel_batch, frames, coords) in enumerate(tqdm(gen, 
		...
		for p, f, c in zip(pred, frames, coords):
			...
			f[y1:y2, x1:x2] = blend_mouth_only(p, original)
			out.write(f)

	out.release()

	command = 'ffmpeg -y -i {} -i {} -c:v libx264 -preset slow -crf 12 -pix_fmt yuv420p -c:a aac -shortest {}'.format(
