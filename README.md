...
| QuickTalk 实时路径 | 本地实时数字人、LLM 对话演示 | `OPENTALKING_DEFAULT_MODEL=quicktalk`，Avatar `model_type=quicktalk` | Avatar manifest 需要 `metadata.asset_root` 和 `metadata.template_video`；可用 `opentalking-quicktalk-bench` 做本地 benchmark |
...
| `quicktalk` | template video + audio | 本地实时 talking-head 适配器；支持 Worker 缓存、流式渲染和 `/chat` 对话链路 |
