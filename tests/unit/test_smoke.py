...
def test_demo_avatar_valid() -> None:
    ...
    demo = root / "examples" / "avatars" / "demo-wav2lip"
...
def test_parse_demo_manifest() -> None:
    root = Path(__file__).resolve().parents[2]
    m = parse_manifest(root / "examples" / "avatars" / "demo-wav2lip" / "manifest.json")
    assert m.id == "demo-wav2lip"
...
def test_musetalk_new_idle_frame_uses_full_frame_sequence() -> None:
    ...
    assert "gesture_config" not in state.extra
...
def test_musetalk_new_static_has_prepared_assets_and_compose_override() -> None:
    root = Path(__file__).resolve().parents[2]
    avatar_dir = root / "examples" / "avatars" / "musetalk_new_static"

    assets = resolve_prepared_musetalk_assets(avatar_dir)
    assert assets is not None
    assert len(assets.coords) == 1
    assert len(assets.latents) == 1
