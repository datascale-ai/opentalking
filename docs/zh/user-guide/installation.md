...
| 持续集成 | CPU | Docker Compose 默认 profile | [Docker Compose → CPU profile](install-with-docker.md#cpu-profile) |
| 单 GPU 机器评估 | NVIDIA 3090 / 4090 / A100（CUDA 12） | Docker Compose GPU profile | [Docker Compose → GPU profile](install-with-docker.md#gpu-profile) |
| 昇腾 NPU 评估 | 华为 910B（CANN 8.0+） | 源码安装 + OmniRT NPU 运行时 | [源码安装 → 昇腾 910B](install-from-source.md#ascend-910b) |
...
- [Docker Compose](install-with-docker.md) —— 使用打包 Docker 栈安装。覆盖 CPU、GPU、昇腾 profile。
