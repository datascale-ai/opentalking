...
用户视角的最短路径：OpenTalking 负责实时数字人编排（LLM / STT / TTS 全部调用百炼 API），FlashTalk 视频生成模型由 [OmniRT](https://github.com/datascale-ai/omnirt) 启动 FlashTalk-compatible WebSocket 服务。整条链路只需要在本地部署 **一个** 模型服务（FlashTalk WebSocket）。
